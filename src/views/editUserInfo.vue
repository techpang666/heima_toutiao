<template>
	<div class="editUserInfo">
		...
		<MyCell title="昵称" :desc="current.nickname"></MyCell>
...
export default {
	...
	methods: {
		async afterRead(myFile) {
			...
			console.log(res);
			if (res.data.data.message == '文件上传成功') {
	...
	async mounted() {
		...
		if (res.data.data.message == '获取成功') {
			// 改造数据
			res.res.data.data.head_img =
				axios.defaults.baseURL + res.res.data.data.head_img;
			// 赋值给current
